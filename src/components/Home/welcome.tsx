...
const cards = [
    {
        icon: "flaticon flaticon-management",
        title: "Our Belief",
...
function WelcomeCard({ icon, title, desc }: { icon: string; title: string; desc: string }) {
    return (
        <div className="bg-[#fff] m-[15px] rounded-md animate__animated animate__slower animate__zoomIn hover:border-b-4 hover:border-[#ff6f00] transition-shadow duration-300 group relative flex flex-col items-center min-w-[260px] max-w-[320px] flex-1 ">
            ...
            <div className="relative h-[67px] w-[67px] bg-[#1D4783] rounded-full flex items-center justify-center z-10 mt-2 mb-2">
                <i className={`${icon} text-white text-3xl`}></i>
            </div>
            <h3 className="text-[#223645] font-bold text-xl mb-2">{title}</h3>
            <p className="text-[#7a8a9e] text-center px-2 mb-4">{desc}</p>
        </div>
    );
}
...
function Welcome() {
    return (
   <div className="site-main bg-[#f7f5f5]">
  <section className="py-12 md:py-20 lg:py-[72px]">
